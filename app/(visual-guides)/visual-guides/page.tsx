...
const title = "Visual City Guides";
const description =
  "Browse source-attributed visual city guides with verified media context, city intelligence links, arrival planning, neighborhood research, moving-to planning, comparisons, tools, methodology, and source transparency.";
const introParagraph =
  "Each guide pairs verified Wikimedia imagery from the existing media catalog with structured city intelligence, arrival planning, neighborhood research, and moving-to research. These pages are visual orientation only — they are not a tourism guide, not an attractions ranking, not an official tourism page, and not evidence of current local conditions. Imagery is orientation, not evidence — use the linked city profiles, country hubs, methodology, and data sources for structured context.";

export const metadata: Metadata = createMetadata({
  title,
  description,
  path: staticRoutes.visualGuides,
});

export default function VisualGuidesDirectoryPage() {
  const visualPages = getAllVisualCityGuidePages();
  ...
  const itemListSchema = {
    ...
    itemListElement: guides.map((entry) => ({
      "@type": "ListItem",
      name: `Visual Guide to ${entry.city.name}`,
      url: absoluteUrl(visualCityGuideRoute(entry.city.slug)),
    })),
  };
  ...
  return (
    <main>
      ...
      <PageHeader eyebrow="Visual guides" intro={introParagraph} title={title}>
      ...
      <Container className="space-y-12 py-12">
        ...
        <section
          aria-labelledby="visual-directory-scope-heading"
          className="rounded-2xl border border-neutral-border bg-surface-soft p-6"
        >
          <h2
            className="text-2xl font-semibold text-text-primary"
            id="visual-directory-scope-heading"
          >
            Sources and methodology
          </h2>
          <p className="mt-3 max-w-3xl text-base leading-7 text-text-secondary">
            Visual guides use verified Wikimedia hero (and where available,
            secondary) imagery from the existing media catalog, with full
            source, author, and license attribution. They do not publish
            attraction rankings, neighborhood names, district boundaries, rent
            or sale prices, crime rates, school rankings, hospital proximities,
            transit operators, or any &ldquo;best&rdquo; /
            &ldquo;must-see&rdquo; / &ldquo;safest&rdquo; /
            &ldquo;cheapest&rdquo; claims. Read the{" "}
            <Link
                ...
                <li key={page.citySlug}>
                  <Card as="article" className="h-full" interactive>
                    ...
                    <p className="mt-1 text-xs font-medium text-text-secondary">
                      {getVisualGuideFocusLabel(page.visualFocus)}
                    </p>
                    ...
                    <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-sm">
                      <Link
                        ...
                        href={visualCityGuideRoute(city.slug)}
                      >
                        {`Open the ${city.name} visual guide`}
                      </Link>
                      <Link
        ...
        <section aria-labelledby="visual-directory-country-heading">
          <SectionHeading
            description="A complete index of visual city guides grouped by country, fully present in the initial HTML so every link is crawlable without client-side JavaScript."
            title="Visual guides by country"
          />
          <h2 className="sr-only" id="visual-directory-country-heading">
            Visual guides by country
          </h2>
        ...
        <section
          aria-labelledby="visual-directory-related-heading"
          className="rounded-2xl border border-neutral-border bg-surface-soft p-6"
        >
          <h2
            className="text-2xl font-semibold text-text-primary"
            id="visual-directory-related-heading"
          >
            Continue exploring
          </h2>
          <p className="mt-3 max-w-3xl text-base leading-7 text-text-secondary">
            Use the structured city and country profiles, comparisons,
            arrival, neighborhood, and moving-to planning alongside any visual
            guide. Calculators use your own inputs and do not query housing,
            salary, or transport providers — they do not publish live prices
            or schedules.
          </p>
          <ul className="mt-5 grid gap-3 text-sm md:grid-cols-2">
            ...
            <li>
              ...
              <span className="text-text-secondary">
                {" "}
                — first-day planning context paired with visual orientation.
              </span>
            </li>
            <li>
              <Link
              ...
              <span className="text-text-secondary">
                {" "}
                — relocation research paired with visual orientation.
              </span>
            </li>
            <li>
              <Link
