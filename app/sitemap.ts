...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const costOfLivingItems = cities.map((city) => ({
    url: absoluteUrl(costOfLivingRoute(city.slug)),
    lastModified: new Date(city.lastUpdated),
    changeFrequency: "monthly" as const,
    priority: 0.7,
  }));
  ...
  const economyItems = cities.map((city) => ({
    url: absoluteUrl(economyRoute(city.slug)),
    lastModified: new Date(city.lastUpdated),
    changeFrequency: "monthly" as const,
    priority: 0.72,
  }));

  const educationItems = cities.map((city) => ({
    url: absoluteUrl(educationRoute(city.slug)),
    lastModified: new Date(city.lastUpdated),
    changeFrequency: "monthly" as const,
    priority: 0.72,
  }));
  ...
  const regionalCollectionItems = [
    {
      url: absoluteUrl(staticRoutes.regionalCollections),
      lastModified: staticFreshness,
      changeFrequency: "monthly" as const,
      priority: 0.8,
    },
