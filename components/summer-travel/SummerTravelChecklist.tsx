...
interface SummerTravelChecklistProps {
  items: readonly SummerTravelChecklistItem[];
}
...
const CATEGORY_ORDER: readonly CategoryDefinition[] = [
  {
    ...
    label: "Arrival and first-day planning",
    description:
  ...
  },
  {
    ...
    label: "Weather and seasonal preparation",
    description:
...
export function SummerTravelChecklist({ items }: SummerTravelChecklistProps) {
  return (
    <div className="space-y-8">
      {CATEGORY_ORDER.map((group) => {
        ...
        return (
          <section
            aria-labelledby={`summer-checklist-${group.category}`}
            key={group.category}
          >
            <h3
              className="text-base font-semibold text-text-primary"
              id={`summer-checklist-${group.category}`}
            >
              {group.label}
            </h3>
