#!/usr/bin/env python3
"""Wave 16 wiring: splice 250 resolved cities (SE/NO/DK/FI/RO/HR/SK) + heroes + nearby
into repo data files. Deterministic; reads /tmp/w16/{selected,heroes,nearby}.json.
Append-only. Anchors match current (post-wave15) file state. New const names:
cityImagesWave16 / wave16NearbySeeds. TIER must match phase6_append.py exactly."""
import json, hashlib
...
heroes = json.load(open("/tmp/w16/heroes.json"))
nearby = json.load(open("/tmp/w16/nearby.json"))

COUNTRY_NAME = {"sweden": "Sweden", "norway": "Norway", "denmark": "Denmark",
                "finland": "Finland", "romania": "Romania", "croatia": "Croatia", "slovakia": "Slovakia"}
REGION = {"sweden": "Northern Europe", "norway": "Northern Europe",
          "denmark": "Northern Europe", "finland": "Northern Europe",
          "romania": "Eastern Europe", "croatia": "Southeastern Europe", "slovakia": "Central Europe"}

def pop_label(p):
    p = float(p)
    return f"~{p/1e6:.1f}M" if p >= 1e6 else f"~{round(p/1000)}K"

cities = [{"slug": c["slug"], "name": c["name"], "qid": c["qid"],
           ...
           "hero": heroes.get(c["slug"])} for c in sel]
...
TIER = {"sweden": (79, 46, 82, 82, 82), "norway": (80, 42, 84, 84, 84),
        "denmark": (79, 46, 82, 83, 82), "finland": (79, 47, 84, 82, 83),
        "romania": (72, 66, 66, 70, 68), "croatia": (74, 60, 74, 74, 72),
        "slovakia": (74, 62, 70, 74, 72)}
DEF = (62, 60, 62, 62, 60)

# ---------- 1. cities.ts ----------
city_blocks = [f"  // ===== Wave 16: +250 cities (SE/NO/DK/FI/RO/HR/SK), A-F complete ({DATE}) ====="]
for c in cities:
...
# ---------- 2. city-images.ts ----------
hero_blocks = [f"  // ===== Wave 16 heroes ({DATE}) ====="]
hn = 0
for c in cities:
    h = c.get("hero")
    if not h: continue
...
concat = "export const cityImages: PlaceImage[] = [...cityImagesBase, ...cityImagesWave8, ...cityImagesWave9, ...cityImagesWave10, ...cityImagesWave11, ...cityImagesWave12, ...cityImagesWave13, ...cityImagesWave14, ...cityImagesWave15];"
assert img_src.count(concat) == 1, f"cityImages concat count={img_src.count(concat)}"
new_block = ("const cityImagesWave16: PlaceImage[] = [\n" + "\n".join(hero_blocks) + "\n];\n\n"
             "export const cityImages: PlaceImage[] = [...cityImagesBase, ...cityImagesWave8, ...cityImagesWave9, ...cityImagesWave10, ...cityImagesWave11, ...cityImagesWave12, ...cityImagesWave13, ...cityImagesWave14, ...cityImagesWave15, ...cityImagesWave16];")
img_src = img_src.replace(concat, new_block)
(ROOT / "lib/data/media/city-images.ts").write_text(img_src)
print(f"city-images.ts: +{hn} heroes (cityImagesWave16 const)")

# ---------- 3. nearby-places.ts ----------
seed_lines = ["const wave16NearbySeeds: readonly PlaceSeed[] = ["]
img_lines = [f"  // ===== Wave 16 verified images ({DATE}) ====="]
facts_lines = [f"  // ===== Wave 16 facts ({DATE}) ====="]
detail_slugs = []
place_total = 0
...
print(f"nearby-places.ts: +{place_total} seeds + images (wave16NearbySeeds const)")
...
det_block = f"  // ===== Wave 16 verified detail pages ({DATE}) =====\n" + "\n".join(f"  {j(s)}," for s in detail_slugs)
idx = det_src.rfind("\n] as const;")
...
wt_lines = [f"  // ===== Wave 16 ({DATE}) ====="]
vg_lines = [f"  // ===== Wave 16 ({DATE}) ====="]
