#!/usr/bin/env python3
"""Wave 14 wiring: splice 250 resolved cities (US/UK/DE/BE/AT/CH) + heroes + nearby
places into repo data files. Deterministic; reads /tmp/w14/{selected,heroes,nearby}.json.
Append-only. Anchors match current (post-wave13) file state. New const names:
cityImagesWave14 / wave14NearbySeeds. TIER must match phase6_append.py exactly."""
import json, hashlib
from pathlib import Path

ROOT = Path("/Users/agent/global-city-intelligence")
DATE = "2026-07-09"
sel = json.load(open("/tmp/w14/selected.json"))
heroes = json.load(open("/tmp/w14/heroes.json"))
nearby = json.load(open("/tmp/w14/nearby.json"))
...
REGION = {"united-states": "North America", "united-kingdom": "Western Europe",
          "germany": "Central Europe", "belgium": "Western Europe",
          "austria": "Central Europe", "switzerland": "Western Europe"}
...
TIER = {}
for c in ["germany", "united-kingdom", "austria"]:
    TIER[c] = (78, 48, 78, 80, 80)
TIER["united-states"] = (76, 52, 70, 78, 72)
TIER["belgium"] = (78, 50, 76, 79, 79)
TIER["switzerland"] = (79, 40, 82, 81, 83)
DEF = (62, 60, 62, 62, 60)

# ---------- 1. cities.ts seed blocks ----------
city_blocks = [f"  // ===== Wave 14: +250 cities (US/UK/DE/BE/AT/CH), A-F complete ({DATE}) ====="]
for c in cities:
...
hero_blocks = [f"  // ===== Wave 14 heroes ({DATE}) ====="]
hn = 0
for c in cities:
    h = c.get("hero")
    if not h: continue
...
concat = "export const cityImages: PlaceImage[] = [...cityImagesBase, ...cityImagesWave8, ...cityImagesWave9, ...cityImagesWave10, ...cityImagesWave11, ...cityImagesWave12, ...cityImagesWave13];"
assert img_src.count(concat) == 1, f"cityImages concat count={img_src.count(concat)}"
new_block = ("const cityImagesWave14: PlaceImage[] = [\n" + "\n".join(hero_blocks) + "\n];\n\n"
             "export const cityImages: PlaceImage[] = [...cityImagesBase, ...cityImagesWave8, ...cityImagesWave9, ...cityImagesWave10, ...cityImagesWave11, ...cityImagesWave12, ...cityImagesWave13, ...cityImagesWave14];")
img_src = img_src.replace(concat, new_block)
(ROOT / "lib/data/media/city-images.ts").write_text(img_src)
print(f"city-images.ts: +{hn} heroes (cityImagesWave14 const)")

# ---------- 3. nearby-places.ts: wave14NearbySeeds + VERIFIED_IMAGES ----------
seed_lines = ["const wave14NearbySeeds: readonly PlaceSeed[] = ["]
img_lines = [f"  // ===== Wave 14 verified images ({DATE}) ====="]
facts_lines = [f"  // ===== Wave 14 facts ({DATE}) ====="]
detail_slugs = []
place_total = 0
...
print(f"nearby-places.ts: +{place_total} seeds + images (wave14NearbySeeds const)")
...
det_block = f"  // ===== Wave 14 verified detail pages ({DATE}) =====\n" + "\n".join(f"  {j(s)}," for s in detail_slugs)
idx = det_src.rfind("\n] as const;")
...
wt_lines = [f"  // ===== Wave 14 ({DATE}) ====="]
vg_lines = [f"  // ===== Wave 14 ({DATE}) ====="]
