...
# ensure every NEW selected city has coords (all_city_coords may predate the final selection)
for _c in sel:
    coords[_c["slug"]] = [round(_c["lat"], 5), round(_c["lon"], 5)]
...
# Only link to REAL cities. all_city_coords.json can retain stale candidate slugs
# (excluded/renamed picks like villa-de-alvarez / santacruz-co); restrict the link
# universe to slugs that actually exist in cities.ts (existing + wave19).
_cs = (ROOT / "lib/data/cities.ts").read_text()
_body = _cs[_cs.index("const seeds: CitySeed[] = ["):_cs.index("export const cities")]
valid_slugs = set(re.findall(r'slug: "([a-z0-9][a-z0-9-]*)"', _body))
pool = [(s, v[0], v[1]) for s, v in coords.items() if s in valid_slugs]
def edges_for(slug):
...
blocks = [f'  // ===== Wave 19 ({len(sel)} new cities) =====']
total_edges = 0
for c in sel:
    es = edges_for(c["slug"])
    total_edges += len(es)
