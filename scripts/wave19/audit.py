...
if len(slugs) != 4444: fail.append(f"city count {len(slugs)} != 3744")
...
badh = []   # quality defects on heroes that EXIST -> fail
missh = []  # no acceptable Commons image -> justified fallback (media validator does not require a hero) -> warn
for s in new:
    h = heroes.get(s)
    if not h: missh.append(s); continue
...
if badh: fail.append(f"hero quality issues: {badh[:6]}")
if missh: warn.append(f"cities without hero (justified fallback — no acceptable CC/PD Commons image): {missh}")
...
for needconst in ("wave19NearbySeeds", "wave19SparseSeeds"):
