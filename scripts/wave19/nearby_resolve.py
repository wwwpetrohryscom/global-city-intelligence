...
slug dedup + within-city QID dedup. Checkpoints /tmp/w19/nearby.json (resumable)."""
...
BAD_NAME = re.compile(r"memorial|monument|\bhistoric\b|heritage|\bmall\b|battlefield|\bfort\b|\bprison\b|"
    ...
    r"capitol|courthouse|\bhall\b|cathedral|\bchurch\b|\bdam\b|parkway|bathing beach|arabian peninsula|indian subcontinent|\barabia\b|\beurasia\b|\bafrica\b|arabian plate|\bairport\b|aeroporto|aeropuerto|south america|américa do sul|américa del sur|amazon basin|\bsouth american\b|latin america|\bairport\b|aerodrome|air base|palm jumeirah|palm jebel|\bthe world\b|world islands|amwaj|passport island|deira island|jumeirah island|reclaimed|marina city|deira island|jumeirah islands|\bmarina\b", re.I)
...
METROS = [(52.370,4.895),(51.924,4.478),(52.078,4.310),(51.441,5.469),  # Amsterdam/Rotterdam/The Hague/Eindhoven
          (40.417,-3.703),(41.385,2.173),(39.470,-0.376),(37.389,-5.984),(43.263,-2.935),  # Madrid/Barcelona/Valencia/Sevilla/Bilbao
          (41.902,12.496),(45.464,9.190),(40.852,14.268),(45.438,10.992),(45.070,7.687),(43.770,11.256),  # Rome/Milan/Naples/Verona/Turin/Florence
          (38.722,-9.139),(41.158,-8.629),  # Lisbon/Porto
          (52.230,21.012),(50.065,19.945),(51.107,17.038),(54.352,18.646),(52.407,16.925),  # Warsaw/Krakow/Wroclaw/Gdansk/Poznan
          (50.076,14.438),(49.195,16.608)]  # Prague/Brno
def hav(a, b, c, d):
    r = math.radians
...
pool = []
for cc in ["brazil", "mexico", "argentina", "colombia", "chile", "peru", "ecuador", "bolivia", "uruguay", "paraguay", "costa-rica", "panama", "dominican-republic"]:
...
def verify_image(f, name):
    ...
    if max(ii.get("ow") or 0, ii.get("oh") or 0) < 600: return None
...
def resolve_city(city, used):
    ...
    cands = []
    for f in pool:
        if abs(f["lat"]-clat) > 2.3 or abs(f["lon"]-clon) > 3.0: continue
