#!/usr/bin/env python3
"""Wave 19 sparse-expansion wiring: append topped-up nearby (sparse_add.json) into
nearby-places.ts (wave19SparseSeeds), facts, detail-slugs. Targets post-wave19-wire
anchor `...wave19NearbySeeds];`."""
import json
...
seed_lines = ["const wave19SparseSeeds: readonly PlaceSeed[] = ["]
img_lines = [f"  // ===== Wave 19 sparse-expansion verified images ({DATE}) ====="]
facts_lines = [f"  // ===== Wave 19 sparse-expansion facts ({DATE}) ====="]
detail_slugs = []
place_total = 0
...
assert "const wave19SparseSeeds" not in np_src, "sparse seeds already wired"
decl_idx = np_src.index("const seeds: readonly PlaceSeed[] = [")
np_src = np_src[:decl_idx] + "\n".join(seed_lines) + "\n\n" + np_src[decl_idx:]
np_src = np_src.replace(anchor, "...wave19NearbySeeds, ...wave19SparseSeeds];")
...
print(f"nearby-places.ts: +{place_total} sparse seeds + images (wave19SparseSeeds)")
...
det_block = f"  // ===== Wave 19 sparse-expansion detail pages ({DATE}) =====\n" + "\n".join(f"  {j(s)}," for s in detail_slugs)
idx = det_src.rfind("\n] as const;")
