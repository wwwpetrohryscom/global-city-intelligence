...
--dry prints eligibility + samples; without it, writes /tmp/w19/selected.json."""
...
for c in existing_cities:
    exist_names_by_country.setdefault(c["countrySlug"], set()).add(
        re.sub(r"[^a-z0-9]", "", _deacc(c["name"]).lower()))
...
CC = {"brazil":"br","mexico":"mx","argentina":"ar","colombia":"co","chile":"cl","peru":"pe","ecuador":"ec","bolivia":"bo","uruguay":"uy","paraguay":"py","costa-rica":"cr","panama":"pa","dominican-republic":"do"}
...
HARD_TYPE = re.compile(r"metropolitan|functional urban area|urban agglomeration|agglomeration|"
    ...
    r"\bpartido\b|\bprovincia\b|departamento|\bcomarca\b|meso-?region|micro-?region|intermediate geographic region|immediate geographic region|region metropolitana|área metropolitana|former municipality|ghost town|abandoned")
SOFT_TYPE = re.compile(r"locality of|ortsteil|stadtteil|\bquarter\b|borough of|subdivision|"
    ...
    r"\btownship\b|\bcommune of\b|\bcorregimiento\b|first-level administrative division")
CITYISH = re.compile(r"\bcity\b|\btown\b|municipality|commune|\bstad\b|gemeinde|"
    ...
    r"county-level|sub-provincial|special city|metropolitan city|municipio|munic[ií]pio|\bciudad\b|\bvila\b|comuna")
BAD_NAME = re.compile(r"metropolitan|metro area|agglomeration|urban area|conurbation|randstad|"
    ...
    r"\bemirate\b|\bprovince\b|zona metropolitana|region metropolitana|gran buenos aires|área metropolitana", re.I)

def mainland_ok(slug, lat, lon):
    """Country bounding boxes (reject stray/overseas/wrong-country coords)."""
    B = {"brazil": (-33.8, 5.3, -74.1, -34.7), "mexico": (14.4, 32.8, -118.5, -86.7),
         "argentina": (-55.2, -21.7, -73.7, -53.5), "colombia": (-4.3, 13.0, -79.1, -66.8),
         "chile": (-56.0, -17.4, -75.8, -66.3), "peru": (-18.4, 0.1, -81.4, -68.6),
         "ecuador": (-5.1, 1.5, -81.1, -75.1), "bolivia": (-23.0, -9.6, -69.7, -57.4),
         "uruguay": (-35.1, -30.0, -58.5, -53.0), "paraguay": (-27.7, -19.2, -62.7, -54.2),
         "costa-rica": (8.0, 11.3, -86.0, -82.5), "panama": (7.0, 9.8, -83.2, -77.1),
         "dominican-republic": (17.4, 20.1, -72.1, -68.2)}
    if slug in B:
        a, b, c, d = B[slug]
...
norm = lambda s: re.sub(r"[^a-z0-9]", "", _deacc(s).lower())
...
def disputed(r):
    ...
    # Malvinas / Falkland Islands (UK-administered, Argentina-claimed) -> exclude as ambiguous
    if cc == "argentina" and -52.7 <= r["lat"] <= -50.9 and -61.6 <= r["lon"] <= -57.5: return True
    return False
...
def eligible(r):
    ...
    _sm = re.sub(r"[^a-z0-9]","", re.sub(r"\b(municipality|municipio|munic[ií]pio|ciudad|gran|comuna|cantón|canton|partido|departamento|kommun|kommune|kunta)\b","",_deacc(r["name"]).lower()))
    if _sm in {re.sub(r"(municipality|municipio|munic[ií]pio|ciudad|gran|comuna|cantón|canton|partido|departamento|kommun|kommune|kunta)","",x) for x in exist_names_by_country.get(r["countrySlug"], set())}: return None
...
CAPS = json.load(open(OUT / "caps.json")) if (OUT / "caps.json").exists() else {
    "brazil": 100, "mexico": 60, "argentina": 40, "colombia": 40, "chile": 25, "peru": 25, "ecuador": 15, "bolivia": 10, "uruguay": 10, "paraguay": 10, "costa-rica": 5, "panama": 5, "dominican-republic": 5}
ORDER = ["brazil", "mexico", "colombia", "argentina", "peru", "chile", "ecuador", "bolivia", "uruguay", "paraguay", "dominican-republic", "costa-rica", "panama"]
TARGET = 350
selected = []
used_slugs = set()
...
print("wrote /tmp/w19/selected.json")
