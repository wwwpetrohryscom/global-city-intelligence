#!/usr/bin/env python3
"""Wave 19 wiring: splice 375 resolved cities (15 countries) + heroes + nearby into
repo data files. Deterministic; reads /tmp/w19/{selected,heroes,nearby}.json. Append-only.
...
heroes = json.load(open("/tmp/w19/heroes.json"))
nearby = json.load(open("/tmp/w19/nearby.json"))
...
REGION = {"brazil": "South America", "mexico": "North America", "argentina": "South America",
          ...
          "paraguay": "South America", "costa-rica": "Central America", "panama": "Central America",
          "dominican-republic": "Caribbean"}
TIER = {"brazil": (66, 64, 62, 72, 70), "mexico": (64, 64, 58, 72, 66), "argentina": (68, 66, 66, 66, 72),
        "colombia": (60, 66, 58, 66, 62), "chile": (72, 58, 66, 74, 70), "peru": (58, 68, 56, 64, 60),
        "ecuador": (60, 62, 60, 60, 58), "bolivia": (54, 72, 54, 56, 54), "uruguay": (74, 56, 74, 68, 66),
        "paraguay": (56, 72, 60, 58, 54), "costa-rica": (72, 58, 78, 68, 62), "panama": (68, 58, 66, 76, 60),
        "dominican-republic": (60, 62, 62, 66, 56)}
DEF = (62, 60, 62, 62, 60)
...
# ---------- 1. cities.ts ----------
city_blocks = [f"  // ===== Wave 19: +350 cities (13 countries BR/MX/AR/CO/CL/PE/EC/BO/UY/PY/CR/PA/DO), A-F complete ({DATE}) ====="]
for c in cities:
...
# ---------- 2. city-images.ts ----------
hero_blocks = [f"  // ===== Wave 19 heroes ({DATE}) ====="]
hn = 0
for c in cities:
    h = c.get("hero")
    if not h: continue
...
concat = "export const cityImages: PlaceImage[] = [...cityImagesBase, ...cityImagesWave8, ...cityImagesWave9, ...cityImagesWave10, ...cityImagesWave11, ...cityImagesWave12, ...cityImagesWave13, ...cityImagesWave14, ...cityImagesWave15, ...cityImagesWave16, ...cityImagesWave17, ...cityImagesWave18];"
assert img_src.count(concat) == 1, f"cityImages concat count={img_src.count(concat)}"
new_block = ("const cityImagesWave19: PlaceImage[] = [\n" + "\n".join(hero_blocks) + "\n];\n\n"
             "export const cityImages: PlaceImage[] = [...cityImagesBase, ...cityImagesWave8, ...cityImagesWave9, ...cityImagesWave10, ...cityImagesWave11, ...cityImagesWave12, ...cityImagesWave13, ...cityImagesWave14, ...cityImagesWave15, ...cityImagesWave16, ...cityImagesWave17, ...cityImagesWave18, ...cityImagesWave19];")
img_src = img_src.replace(concat, new_block)
(ROOT / "lib/data/media/city-images.ts").write_text(img_src)
print(f"city-images.ts: +{hn} heroes (cityImagesWave19 const)")

# ---------- 3. nearby-places.ts ----------
seed_lines = ["const wave19NearbySeeds: readonly PlaceSeed[] = ["]
img_lines = [f"  // ===== Wave 19 verified images ({DATE}) ====="]
facts_lines = [f"  // ===== Wave 19 facts ({DATE}) ====="]
detail_slugs = []
place_total = 0
...
print(f"nearby-places.ts: +{place_total} seeds + images (wave19NearbySeeds const)")
...
det_block = f"  // ===== Wave 19 verified detail pages ({DATE}) =====\n" + "\n".join(f"  {j(s)}," for s in detail_slugs)
idx = det_src.rfind("\n] as const;")
...
wt_lines = [f"  // ===== Wave 19 ({DATE}) ====="]
vg_lines = [f"  // ===== Wave 19 ({DATE}) ====="]
