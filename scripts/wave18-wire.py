#!/usr/bin/env python3
"""Wave 18 wiring: splice 375 resolved cities (15 countries) + heroes + nearby into
repo data files. Deterministic; reads /tmp/w18/{selected,heroes,nearby}.json. Append-only.
...
heroes = json.load(open("/tmp/w18/heroes.json"))
nearby = json.load(open("/tmp/w18/nearby.json"))
...
TIER = {"japan": (82, 58, 68, 82, 84), "south-korea": (80, 56, 62, 82, 82), "china": (66, 64, 50, 78, 74),
        "india": (58, 70, 46, 68, 66), "pakistan": (54, 70, 44, 58, 54), "iran": (58, 72, 54, 56, 60),
        "iraq": (52, 66, 50, 56, 48), "saudi-arabia": (68, 62, 58, 78, 60), "oman": (72, 60, 66, 72, 56),
        "united-arab-emirates": (78, 50, 60, 84, 66), "cyprus": (76, 62, 78, 70, 66)}
DEF = (62, 60, 62, 62, 60)
...
# ---------- 1. cities.ts ----------
city_blocks = [f"  // ===== Wave 18: +350 cities (11 countries JP/KR/CN/IN/PK/IR/IQ/SA/OM/AE/CY), A-F complete ({DATE}) ====="]
for c in cities:
...
# ---------- 2. city-images.ts ----------
hero_blocks = [f"  // ===== Wave 18 heroes ({DATE}) ====="]
hn = 0
for c in cities:
    h = c.get("hero")
    if not h: continue
...
concat = "export const cityImages: PlaceImage[] = [...cityImagesBase, ...cityImagesWave8, ...cityImagesWave9, ...cityImagesWave10, ...cityImagesWave11, ...cityImagesWave12, ...cityImagesWave13, ...cityImagesWave14, ...cityImagesWave15, ...cityImagesWave16, ...cityImagesWave17];"
assert img_src.count(concat) == 1, f"cityImages concat count={img_src.count(concat)}"
new_block = ("const cityImagesWave18: PlaceImage[] = [\n" + "\n".join(hero_blocks) + "\n];\n\n"
             "export const cityImages: PlaceImage[] = [...cityImagesBase, ...cityImagesWave8, ...cityImagesWave9, ...cityImagesWave10, ...cityImagesWave11, ...cityImagesWave12, ...cityImagesWave13, ...cityImagesWave14, ...cityImagesWave15, ...cityImagesWave16, ...cityImagesWave17, ...cityImagesWave18];")
img_src = img_src.replace(concat, new_block)
(ROOT / "lib/data/media/city-images.ts").write_text(img_src)
print(f"city-images.ts: +{hn} heroes (cityImagesWave18 const)")

# ---------- 3. nearby-places.ts ----------
seed_lines = ["const wave18NearbySeeds: readonly PlaceSeed[] = ["]
img_lines = [f"  // ===== Wave 18 verified images ({DATE}) ====="]
facts_lines = [f"  // ===== Wave 18 facts ({DATE}) ====="]
detail_slugs = []
place_total = 0
...
new_decl = ("\n".join(seed_lines) + "\n\n" + seeds_decl.replace("...wave17NearbySeeds];", "...wave17NearbySeeds, ...wave18NearbySeeds];"))
...
print(f"nearby-places.ts: +{place_total} seeds + images (wave18NearbySeeds const)")
...
det_block = f"  // ===== Wave 18 verified detail pages ({DATE}) =====\n" + "\n".join(f"  {j(s)}," for s in detail_slugs)
idx = det_src.rfind("\n] as const;")
...
wt_lines = [f"  // ===== Wave 18 ({DATE}) ====="]
vg_lines = [f"  // ===== Wave 18 ({DATE}) ====="]
