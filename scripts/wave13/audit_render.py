...
sel = {c["slug"]: c for c in json.load(open("/tmp/w13/selected.json"))}
nb = json.load(open("/tmp/w13/nearby.json"))
city_sample = [s["slug"] for s in json.load(open("/tmp/w13/audit_sample.json"))]
for u in ("ayr-uk", "crawley", "salford"):
    if u in sel and u not in city_sample:
        city_sample.append(u)
detail_sample = json.load(open("/tmp/w13/detail_sample.json"))
