...
--dry prints eligibility + samples; without it, writes /tmp/w16/selected.json."""
...
CC = {"sweden":"se","norway":"no","denmark":"dk","finland":"fi","romania":"ro","croatia":"hr","slovakia":"sk"}
...
def mainland_ok(slug, lat, lon):
    ...
    if slug == "sweden":
        return 55.2 <= lat <= 69.2 and 10.8 <= lon <= 24.3
    if slug == "norway":  # mainland (exclude Svalbard/Jan Mayen)
        return 57.8 <= lat <= 71.3 and 4.3 <= lon <= 31.3
    if slug == "denmark":  # mainland + isles (exclude Greenland/Faroe)
        return 54.5 <= lat <= 57.9 and 8.0 <= lon <= 15.3
    if slug == "finland":
        return 59.7 <= lat <= 70.2 and 19.3 <= lon <= 31.7
    if slug == "romania":
        return 43.5 <= lat <= 48.4 and 20.2 <= lon <= 29.8
    if slug == "croatia":
        return 42.3 <= lat <= 46.6 and 13.3 <= lon <= 19.5
    if slug == "slovakia":
        return 47.7 <= lat <= 49.7 and 16.8 <= lon <= 22.6
    return True
...
CAPS = json.load(open(OUT / "caps.json")) if (OUT / "caps.json").exists() else {
    "sweden": 45, "norway": 35, "denmark": 30, "finland": 35, "romania": 50, "croatia": 30, "slovakia": 25}
ORDER = ["romania", "sweden", "finland", "norway", "denmark", "croatia", "slovakia"]
TARGET = 250
selected = []
used_slugs = set()
...
print("wrote /tmp/w16/selected.json")
