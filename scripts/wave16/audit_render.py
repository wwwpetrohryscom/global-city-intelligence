...
sel = {c["slug"]: c for c in json.load(open("/tmp/w16/selected.json"))}
nb = json.load(open("/tmp/w16/nearby.json"))
city_sample = [s["slug"] for s in json.load(open("/tmp/w16/audit_sample.json"))]
for u in ():
    if u in sel and u not in city_sample:
        city_sample.append(u)
detail_sample = json.load(open("/tmp/w16/detail_sample.json"))
