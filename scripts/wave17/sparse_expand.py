...
dedup against what each city already links. Writes /tmp/w17/sparse_add.json."""
import sys, json, re, time
from pathlib import Path
sys.path.insert(0, "/Users/agent/global-city-intelligence/scripts/wave17")
import nearby_resolve as NR

ROOT = Path("/Users/agent/global-city-intelligence")
OUT = Path("/tmp/w17")
TARGET = 5  # minimum nearby per city after expansion
...
NEW = set(c["slug"] for c in json.load(open(OUT / "selected.json")))
feasible = [c for c in sparse if ec[c] in pools and c in NEW]
...
def main():
    ...
    json.dump(still_short, open(OUT / "sparse_still_short.json", "w"))
