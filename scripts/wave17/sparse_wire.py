#!/usr/bin/env python3
"""Wave 17 sparse-expansion wiring: append topped-up nearby (sparse_add.json) into
nearby-places.ts (wave17SparseSeeds), facts, detail-slugs. Targets post-wave17-wire
anchor `...wave17NearbySeeds];`."""
import json
...
seed_lines = ["const wave17SparseSeeds: readonly PlaceSeed[] = ["]
img_lines = [f"  // ===== Wave 17 sparse-expansion verified images ({DATE}) ====="]
facts_lines = [f"  // ===== Wave 17 sparse-expansion facts ({DATE}) ====="]
detail_slugs = []
place_total = 0
...
assert "const wave17SparseSeeds" not in np_src, "sparse seeds already wired"
decl_idx = np_src.index("const seeds: readonly PlaceSeed[] = [")
np_src = np_src[:decl_idx] + "\n".join(seed_lines) + "\n\n" + np_src[decl_idx:]
np_src = np_src.replace(anchor, "...wave17NearbySeeds, ...wave17SparseSeeds];")
...
print(f"nearby-places.ts: +{place_total} sparse seeds + images (wave17SparseSeeds)")
...
det_block = f"  // ===== Wave 17 sparse-expansion detail pages ({DATE}) =====\n" + "\n".join(f"  {j(s)}," for s in detail_slugs)
idx = det_src.rfind("\n] as const;")
