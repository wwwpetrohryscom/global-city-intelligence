#!/usr/bin/env python3
"""Wave 11 wiring: splice 250 resolved cities + heroes + nearby places into repo
data files. Deterministic; reads /tmp/w11/{selected,heroes,nearby}.json.
...
TIER = {}
for c in ["germany", "france", "netherlands"]:
...
city_blocks = [f"  // ===== Wave 11: +250 cities (US/CA/AU/DE/FR/NL), A-F complete ({DATE}) ====="]
for c in cities:
...
print(f"city-images.ts: +{hn} heroes (cityImagesWave11 const)")

# ---------- 3. nearby-places.ts: wave11NearbySeeds + VERIFIED_IMAGES ----------
...
print(f"nearby-places.ts: +{place_total} seeds + images (wave11NearbySeeds const)")
...
wt_lines = [f"  // ===== Wave 11 ({DATE}) ====="]
vg_lines = [f"  // ===== Wave 11 ({DATE}) ====="]
