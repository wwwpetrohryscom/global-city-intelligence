...
data files. Deterministic; reads /tmp/w6_selected.json + /tmp/w6_nearby_clean.json."""
import json, re, hashlib
from pathlib import Path

ROOT = Path("/Users/agent/global-city-intelligence")
cities = json.load(open("/tmp/w6_selected.json"))
nearby = json.load(open("/tmp/w6_nearby_clean.json"))
...
city_blocks = ["  // ===== Wave 6: +50 uncovered SEO cities, A-F complete (2026-06-15) ====="]
for c in cities:
...
hero_blocks = ["  // ===== Wave 6 heroes (2026-06-15) ====="]
heroes = 0
for c in cities:
    h = c.get("hero")
    if not h:
...
# ---------- 3. nearby-places.ts: wave6NearbySeeds + VERIFIED_IMAGES ----------
seed_lines = ["const wave6NearbySeeds: readonly PlaceSeed[] = ["]
img_lines = ["  // ===== Wave 6 verified images (2026-06-15) ====="]
facts_lines = ["  // ===== Wave 6 facts (2026-06-15) ====="]
detail_slugs = []
place_total = 0
...
# insert wave6NearbySeeds const before `const seeds: readonly PlaceSeed[] = [...`
seeds_decl = "const seeds: readonly PlaceSeed[] = [...baseSeeds, ...batchSevenDensitySeeds, ...wave1NearbySeeds, ...wave2NearbySeeds, ...wave3NearbySeeds, ...wave4NearbySeeds, ...wave5NearbySeeds];"
assert seeds_decl in np_src, "seeds decl not found"
np_src = np_src.replace(seeds_decl,
    "\n".join(seed_lines) + "\n\nconst seeds: readonly PlaceSeed[] = [...baseSeeds, ...batchSevenDensitySeeds, ...wave1NearbySeeds, ...wave2NearbySeeds, ...wave3NearbySeeds, ...wave4NearbySeeds, ...wave5NearbySeeds, ...wave6NearbySeeds];")
...
print(f"nearby-places.ts: +{place_total} seeds + {place_total} images (wave6NearbySeeds const)")
...
det_block = "  // ===== Wave 6 verified detail pages (2026-06-15) =====\n" + "\n".join(f"  {j(s)}," for s in detail_slugs)
idx = det_src.rfind("\n] as const;")
...
wt_lines = ["  // ===== Wave 6 (2026-06-15) ====="]
vg_lines = ["  // ===== Wave 6 (2026-06-15) ====="]
