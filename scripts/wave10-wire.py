#!/usr/bin/env python3
"""Wave 10 wiring: splice 100 resolved cities + their nearby places into the repo
data files. Deterministic; reads /tmp/w10_selected.json + /tmp/w10_nearby_clean.json."""
import json, re, hashlib
from pathlib import Path

ROOT = Path("/Users/agent/global-city-intelligence")
cities = json.load(open("/tmp/w10_selected.json"))
nearby = json.load(open("/tmp/w10_nearby_clean.json"))
...
for c in ["romania", "bulgaria", "serbia", "bosnia-and-herzegovina", "ukraine", "moldova", "albania", "montenegro", "russia", "belarus", "georgia", "armenia", "azerbaijan", "kazakhstan", "uzbekistan"]:
...
city_blocks = ["  // ===== Wave 10: +100 cities, A-F complete (2026-06-25) ====="]
for c in cities:
...
hero_blocks = ["  // ===== Wave 10 heroes (2026-06-15) ====="]
heroes = 0
for c in cities:
    h = c.get("hero")
    if not h:
...
# city-images.ts is already split (cityImagesBase + cityImagesWave8 + concat export)
# from Wave 8 -> add a clean cityImagesWave9 const and extend the spread.
concat = "export const cityImages: PlaceImage[] = [...cityImagesBase, ...cityImagesWave8, ...cityImagesWave9];"
assert concat in img_src, "cityImages concat (base+wave8) not found"
new_block = ("const cityImagesWave10: PlaceImage[] = [\n" + "\n".join(hero_blocks) + "\n];\n\n"
             "export const cityImages: PlaceImage[] = [...cityImagesBase, ...cityImagesWave8, ...cityImagesWave9, ...cityImagesWave10];")
img_src = img_src.replace(concat, new_block)
(ROOT / "lib/data/media/city-images.ts").write_text(img_src)
print(f"city-images.ts: +{heroes} heroes (cityImagesWave9 const)")

# ---------- 3. nearby-places.ts: wave8NearbySeeds + VERIFIED_IMAGES ----------
seed_lines = ["const wave10NearbySeeds: readonly PlaceSeed[] = ["]
img_lines = ["  // ===== Wave 10 verified images (2026-06-15) ====="]
facts_lines = ["  // ===== Wave 10 facts (2026-06-15) ====="]
detail_slugs = []
place_total = 0
...
# insert wave8NearbySeeds const before `const seeds: readonly PlaceSeed[] = [...`
seeds_decl = "const seeds: readonly PlaceSeed[] = [...baseSeeds, ...batchSevenDensitySeeds, ...wave1NearbySeeds, ...wave2NearbySeeds, ...wave3NearbySeeds, ...wave4NearbySeeds, ...wave5NearbySeeds, ...wave6NearbySeeds, ...wave7NearbySeeds, ...wave8NearbySeeds, ...wave9NearbySeeds];"
assert seeds_decl in np_src, "seeds decl not found"
np_src = np_src.replace(seeds_decl,
    "\n".join(seed_lines) + "\n\nconst seeds: readonly PlaceSeed[] = [...baseSeeds, ...batchSevenDensitySeeds, ...wave1NearbySeeds, ...wave2NearbySeeds, ...wave3NearbySeeds, ...wave4NearbySeeds, ...wave5NearbySeeds, ...wave6NearbySeeds, ...wave7NearbySeeds, ...wave8NearbySeeds, ...wave9NearbySeeds, ...wave10NearbySeeds];")
...
print(f"nearby-places.ts: +{place_total} seeds + {place_total} images (wave8NearbySeeds const)")
...
det_block = "  // ===== Wave 10 verified detail pages (2026-06-15) =====\n" + "\n".join(f"  {j(s)}," for s in detail_slugs)
idx = det_src.rfind("\n] as const;")
...
wt_lines = ["  // ===== Wave 10 (2026-06-15) ====="]
vg_lines = ["  // ===== Wave 10 (2026-06-15) ====="]
