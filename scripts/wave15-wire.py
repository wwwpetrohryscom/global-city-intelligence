#!/usr/bin/env python3
"""Wave 15 wiring: splice 250 resolved cities (NL/ES/IT/PT/PL/CZ) + heroes + nearby
into repo data files. Deterministic; reads /tmp/w15/{selected,heroes,nearby}.json.
Append-only. Anchors match current (post-wave14) file state. New const names:
cityImagesWave15 / wave15NearbySeeds. TIER must match phase6_append.py exactly."""
import json, hashlib
from pathlib import Path

ROOT = Path("/Users/agent/global-city-intelligence")
DATE = "2026-07-09"
sel = json.load(open("/tmp/w15/selected.json"))
heroes = json.load(open("/tmp/w15/heroes.json"))
nearby = json.load(open("/tmp/w15/nearby.json"))

COUNTRY_NAME = {"netherlands": "Netherlands", "spain": "Spain", "italy": "Italy",
                "portugal": "Portugal", "poland": "Poland", "czechia": "Czechia"}
REGION = {"netherlands": "Western Europe", "spain": "Southern Europe",
...
TIER = {"netherlands": (78, 46, 76, 80, 80), "spain": (76, 58, 74, 74, 72),
        "italy": (75, 55, 72, 72, 70), "portugal": (75, 60, 76, 74, 72),
        "poland": (73, 64, 68, 72, 70), "czechia": (74, 62, 70, 74, 72)}
DEF = (62, 60, 62, 62, 60)

# ---------- 1. cities.ts ----------
city_blocks = [f"  // ===== Wave 15: +250 cities (NL/ES/IT/PT/PL/CZ), A-F complete ({DATE}) ====="]
for c in cities:
...
# ---------- 2. city-images.ts ----------
hero_blocks = [f"  // ===== Wave 15 heroes ({DATE}) ====="]
hn = 0
for c in cities:
    h = c.get("hero")
    if not h: continue
...
concat = "export const cityImages: PlaceImage[] = [...cityImagesBase, ...cityImagesWave8, ...cityImagesWave9, ...cityImagesWave10, ...cityImagesWave11, ...cityImagesWave12, ...cityImagesWave13, ...cityImagesWave14];"
assert img_src.count(concat) == 1, f"cityImages concat count={img_src.count(concat)}"
new_block = ("const cityImagesWave15: PlaceImage[] = [\n" + "\n".join(hero_blocks) + "\n];\n\n"
             "export const cityImages: PlaceImage[] = [...cityImagesBase, ...cityImagesWave8, ...cityImagesWave9, ...cityImagesWave10, ...cityImagesWave11, ...cityImagesWave12, ...cityImagesWave13, ...cityImagesWave14, ...cityImagesWave15];")
img_src = img_src.replace(concat, new_block)
(ROOT / "lib/data/media/city-images.ts").write_text(img_src)
print(f"city-images.ts: +{hn} heroes (cityImagesWave15 const)")

# ---------- 3. nearby-places.ts ----------
seed_lines = ["const wave15NearbySeeds: readonly PlaceSeed[] = ["]
img_lines = [f"  // ===== Wave 15 verified images ({DATE}) ====="]
facts_lines = [f"  // ===== Wave 15 facts ({DATE}) ====="]
detail_slugs = []
place_total = 0
...
print(f"nearby-places.ts: +{place_total} seeds + images (wave15NearbySeeds const)")
...
det_block = f"  // ===== Wave 15 verified detail pages ({DATE}) =====\n" + "\n".join(f"  {j(s)}," for s in detail_slugs)
idx = det_src.rfind("\n] as const;")
...
wt_lines = [f"  // ===== Wave 15 ({DATE}) ====="]
vg_lines = [f"  // ===== Wave 15 ({DATE}) ====="]
