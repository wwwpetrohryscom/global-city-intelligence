#!/usr/bin/env python3
"""Replace wave15 cities lacking a usable verified hero (or >=5 nearby) with stronger
same-country candidates that satisfy BOTH a verified Commons hero (relaxed 600px) AND
>=5 verified nearby. Swaps selected/heroes/nearby."""
import json, sys
sys.path.insert(0, "/Users/agent/global-city-intelligence/scripts/wave11")
sys.path.insert(0, "/Users/agent/global-city-intelligence/scripts/wave15")
import commons as C
import nearby_resolve as NR

OUT = "/tmp/w15"
REPLACE = {"antequera": "spain"}
CN = {"netherlands": "Netherlands", "spain": "Spain", "italy": "Italy",
...
for bad, cc in REPLACE.items():
    for cand in extra[cc]:
        if cand["slug"] in have or cand["slug"] in swaps.values():
            continue
        ...
        break
    if bad not in swaps:
        print(f"  !! no replacement for {bad}"); sys.exit(1)
