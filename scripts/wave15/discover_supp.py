#!/usr/bin/env python3
"""Wave 15 supplement: municipality classes for NL/ES/IT/PT/CZ that are NOT subclasses
of human-settlement Q486972, so the main discover.py Q486972 query misses them. Query
each class directly and merge into raw_<country>.json (dedup by QID). Poland's `miasto`
IS a Q486972 subclass -> covered by the main query, no supplement."""
import sys, json
...
OUT = Path("/tmp/w15")
# country slug -> (country QID, [municipality class QIDs], pop threshold)
SUPP = {
    "netherlands": ("Q55",  ["Q2039348"], 14000),
    "spain":       ("Q29",  ["Q2074737"], 17000),
    "italy":       ("Q38",  ["Q747074"],  17000),
    "portugal":    ("Q45",  ["Q13217644"], 9000),
    "czechia":     ("Q213", ["Q5153359"], 10000),
}
