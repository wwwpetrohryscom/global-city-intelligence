#!/usr/bin/env python3
"""Wave 15 sparse-expansion wiring: append topped-up nearby (sparse_add.json) into
nearby-places.ts (wave15SparseSeeds), facts, detail-slugs. Targets post-wave15-wire
anchor `...wave15NearbySeeds];`."""
import json
...
seed_lines = ["const wave15SparseSeeds: readonly PlaceSeed[] = ["]
img_lines = [f"  // ===== Wave 15 sparse-expansion verified images ({DATE}) ====="]
facts_lines = [f"  // ===== Wave 15 sparse-expansion facts ({DATE}) ====="]
detail_slugs = []
place_total = 0
...
assert "const wave15SparseSeeds" not in np_src, "sparse seeds already wired"
decl_idx = np_src.index("const seeds: readonly PlaceSeed[] = [")
np_src = np_src[:decl_idx] + "\n".join(seed_lines) + "\n\n" + np_src[decl_idx:]
np_src = np_src.replace(anchor, "...wave15NearbySeeds, ...wave15SparseSeeds];")
...
print(f"nearby-places.ts: +{place_total} sparse seeds + images (wave15SparseSeeds)")
...
det_block = f"  // ===== Wave 15 sparse-expansion detail pages ({DATE}) =====\n" + "\n".join(f"  {j(s)}," for s in detail_slugs)
idx = det_src.rfind("\n] as const;")
