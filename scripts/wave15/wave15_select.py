...
--dry prints eligibility + samples; without it, writes /tmp/w15/selected.json."""
...
CC = {"netherlands":"nl","spain":"es","italy":"it","portugal":"pt","poland":"pl","czechia":"cz"}
...
def mainland_ok(slug, lat, lon):
    ...
    if slug == "netherlands":  # mainland (exclude Caribbean NL)
        return 50.5 <= lat <= 53.7 and 3.2 <= lon <= 7.3
    if slug == "spain":  # mainland + Balearic + Canary; exclude Ceuta/Melilla noise
        if 36.0 <= lat <= 43.9 and -9.4 <= lon <= 3.4: return True   # mainland
        if 38.6 <= lat <= 40.1 and 1.1 <= lon <= 4.4: return True    # Balearic
        if 27.6 <= lat <= 29.5 and -18.2 <= lon <= -13.3: return True # Canary
        return False
    if slug == "italy":  # mainland + Sicily + Sardinia
        return 36.6 <= lat <= 47.1 and 6.6 <= lon <= 18.6
    if slug == "portugal":  # mainland only (exclude Azores/Madeira)
        return 36.9 <= lat <= 42.2 and -9.6 <= lon <= -6.1
    if slug == "poland":
        return 49.0 <= lat <= 54.9 and 14.1 <= lon <= 24.2
    if slug == "czechia":
        return 48.5 <= lat <= 51.1 and 12.0 <= lon <= 18.9
    if False:  # (disabled legacy US branch)
...
CAPS = json.load(open(OUT / "caps.json")) if (OUT / "caps.json").exists() else {
    "netherlands": 40, "spain": 50, "italy": 50, "portugal": 35, "poland": 50, "czechia": 25}
ORDER = ["spain", "italy", "poland", "netherlands", "portugal", "czechia"]
TARGET = 250
selected = []
used_slugs = set()
...
print("wrote /tmp/w15/selected.json")
