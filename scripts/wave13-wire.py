#!/usr/bin/env python3
"""Wave 13 wiring: splice 250 resolved cities + heroes + nearby places into repo
data files. Deterministic; reads /tmp/w13/{selected,heroes,nearby}.json.
Append-only: existing rows untouched. Anchors match current (post-wave12/stabilize)
file state. New const names: cityImagesWave13 / wave13NearbySeeds."""
import json, hashlib
from pathlib import Path

ROOT = Path("/Users/agent/global-city-intelligence")
DATE = "2026-07-09"
sel = json.load(open("/tmp/w13/selected.json"))
heroes = json.load(open("/tmp/w13/heroes.json"))
nearby = json.load(open("/tmp/w13/nearby.json"))
...
REGION = {"united-states": "North America", "canada": "North America", "australia": "Oceania",
          "france": "Western Europe", "united-kingdom": "Western Europe"}
...
TIER = {}
for c in ["france", "united-kingdom"]:
...
city_blocks = [f"  // ===== Wave 13: +250 cities (US/CA/UK/AU/FR), A-F complete ({DATE}) ====="]
for c in cities:
...
hero_blocks = [f"  // ===== Wave 13 heroes ({DATE}) ====="]
hn = 0
for c in cities:
    h = c.get("hero")
    if not h: continue
...
concat = "export const cityImages: PlaceImage[] = [...cityImagesBase, ...cityImagesWave8, ...cityImagesWave9, ...cityImagesWave10, ...cityImagesWave11, ...cityImagesWave12];"
assert img_src.count(concat) == 1, f"cityImages concat count={img_src.count(concat)}"
new_block = ("const cityImagesWave13: PlaceImage[] = [\n" + "\n".join(hero_blocks) + "\n];\n\n"
             "export const cityImages: PlaceImage[] = [...cityImagesBase, ...cityImagesWave8, ...cityImagesWave9, ...cityImagesWave10, ...cityImagesWave11, ...cityImagesWave12, ...cityImagesWave13];")
img_src = img_src.replace(concat, new_block)
(ROOT / "lib/data/media/city-images.ts").write_text(img_src)
print(f"city-images.ts: +{hn} heroes (cityImagesWave13 const)")

# ---------- 3. nearby-places.ts: wave13NearbySeeds + VERIFIED_IMAGES ----------
seed_lines = ["const wave13NearbySeeds: readonly PlaceSeed[] = ["]
img_lines = [f"  // ===== Wave 13 verified images ({DATE}) ====="]
facts_lines = [f"  // ===== Wave 13 facts ({DATE}) ====="]
detail_slugs = []
place_total = 0
...
print(f"nearby-places.ts: +{place_total} seeds + images (wave13NearbySeeds const)")
...
det_block = f"  // ===== Wave 13 verified detail pages ({DATE}) =====\n" + "\n".join(f"  {j(s)}," for s in detail_slugs)
idx = det_src.rfind("\n] as const;")
...
wt_lines = [f"  // ===== Wave 13 ({DATE}) ====="]
vg_lines = [f"  // ===== Wave 13 ({DATE}) ====="]
