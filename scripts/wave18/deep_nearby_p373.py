...
def main():
    ...
    me = [f for f in fetch("me", ME) if f["name"] and NR.genuine_nature(f["types"], f["name"])]
    cn = [f for f in fetch("cn", CN) if f["name"] and NR.genuine_nature(f["types"], f["name"])]
    used = set(r["slug"] for v in nb.values() for r in v) | set(NR.existing_nearby_slugs)
    for s, c in deficient:
        pool = cn if c["countrySlug"] == "china" else me
        have = nb.get(s, [])
