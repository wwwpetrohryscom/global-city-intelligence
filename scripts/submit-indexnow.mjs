 ...
 *   node scripts/submit-indexnow.mjs --sitemap            Submit every URL in {SITE_URL}/sitemap.xml
 *   node scripts/submit-indexnow.mjs --sitemap=<url>      Submit every URL in a specific sitemap
...
function parseArgs(argv) {
  const opts = { urls: [], dryRun: false, sitemap: null, file: null };
  for (const arg of argv) {
    if (arg === "--dry-run") opts.dryRun = true;
    else if (arg === "--sitemap") opts.sitemap = "";
...
async function fetchSitemapUrls(sitemapUrl) {
  ...
  console.log(`Sitemap contained ${locs.length} <loc> entries.`);
  return locs;
}
...
Env: INDEXNOW_KEY (required), NEXT_PUBLIC_SITE_URL (host + sitemap base).`);
  ...
  const cfg = {
    ...
    chunkSize: Math.min(opts.chunk || INDEXNOW_MAX_BATCH, INDEXNOW_MAX_BATCH),
    retries: Number.isFinite(opts.retries) ? Math.max(0, opts.retries) : 3,
    timeoutMs: Number.isFinite(opts.timeout) ? opts.timeout : 15000,
  };

  // Gather URLs from whichever source(s) were given.
  let urls = [...opts.urls];
  if (opts.file) urls.push(...(await readUrlsFromFile(opts.file)));
  if (opts.sitemap !== null) urls.push(...(await fetchSitemapUrls(opts.sitemap)));
