#!/usr/bin/env python3
"""Wave 14 sparse-expansion wiring: append topped-up nearby places (sparse_add.json)
into nearby-places.ts (wave14SparseSeeds), facts, detail-slugs — append-only. Targets
the post-wave14-wire anchor `...wave14NearbySeeds];`."""
import json
...
seed_lines = ["const wave14SparseSeeds: readonly PlaceSeed[] = ["]
img_lines = [f"  // ===== Wave 14 sparse-expansion verified images ({DATE}) ====="]
facts_lines = [f"  // ===== Wave 14 sparse-expansion facts ({DATE}) ====="]
detail_slugs = []
place_total = 0
...
anchor = "...wave14NearbySeeds];"
assert np_src.count(anchor) == 1, f"spread anchor count={np_src.count(anchor)}"
assert "const wave14SparseSeeds" not in np_src, "sparse seeds already wired"
decl_idx = np_src.index("const seeds: readonly PlaceSeed[] = [")
np_src = np_src[:decl_idx] + "\n".join(seed_lines) + "\n\n" + np_src[decl_idx:]
np_src = np_src.replace(anchor, "...wave14NearbySeeds, ...wave14SparseSeeds];")
...
print(f"nearby-places.ts: +{place_total} sparse seeds + images (wave14SparseSeeds)")
...
det_block = f"  // ===== Wave 14 sparse-expansion detail pages ({DATE}) =====\n" + "\n".join(f"  {j(s)}," for s in detail_slugs)
idx = det_src.rfind("\n] as const;")
