...
--dry prints eligibility + samples; without it, writes /tmp/w14/selected.json."""
...
CC = {"united-states":"us","united-kingdom":"uk","germany":"de","belgium":"be","austria":"at","switzerland":"ch"}
...
def mainland_ok(slug, lat, lon):
    ...
    if slug == "germany":
        return 47.2 <= lat <= 55.1 and 5.8 <= lon <= 15.1
    if slug == "belgium":
        return 49.4 <= lat <= 51.6 and 2.5 <= lon <= 6.5
    if slug == "austria":
        return 46.3 <= lat <= 49.1 and 9.5 <= lon <= 17.2
    if slug == "switzerland":
        return 45.7 <= lat <= 47.9 and 5.9 <= lon <= 10.6
...
CAPS = json.load(open(OUT / "caps.json")) if (OUT / "caps.json").exists() else {
    "united-states": 50, "united-kingdom": 50, "germany": 50, "belgium": 34, "austria": 33, "switzerland": 33}
ORDER = ["united-states", "united-kingdom", "germany", "belgium", "austria", "switzerland"]
TARGET = 250
selected = []
used_slugs = set()
...
print("wrote /tmp/w14/selected.json")
