#!/usr/bin/env python3
"""Wave 8 wiring: splice 100 resolved cities + their nearby places into the repo
data files. Deterministic; reads /tmp/w8_selected.json + /tmp/w8_nearby_clean.json."""
import json, re, hashlib
from pathlib import Path

ROOT = Path("/Users/agent/global-city-intelligence")
cities = json.load(open("/tmp/w8_selected.json"))
nearby = json.load(open("/tmp/w8_nearby_clean.json"))
...
city_blocks = ["  // ===== Wave 8: +100 cities, A-F complete (2026-06-21) ====="]
for c in cities:
...
hero_blocks = ["  // ===== Wave 8 heroes (2026-06-15) ====="]
heroes = 0
for c in cities:
    h = c.get("hero")
    if not h:
...
seed_lines = ["const wave8NearbySeeds: readonly PlaceSeed[] = ["]
img_lines = ["  // ===== Wave 8 verified images (2026-06-15) ====="]
facts_lines = ["  // ===== Wave 8 facts (2026-06-15) ====="]
detail_slugs = []
place_total = 0
...
seeds_decl = "const seeds: readonly PlaceSeed[] = [...baseSeeds, ...batchSevenDensitySeeds, ...wave1NearbySeeds, ...wave2NearbySeeds, ...wave3NearbySeeds, ...wave4NearbySeeds, ...wave5NearbySeeds, ...wave6NearbySeeds, ...wave7NearbySeeds];"
assert seeds_decl in np_src, "seeds decl not found"
np_src = np_src.replace(seeds_decl,
    "\n".join(seed_lines) + "\n\nconst seeds: readonly PlaceSeed[] = [...baseSeeds, ...batchSevenDensitySeeds, ...wave1NearbySeeds, ...wave2NearbySeeds, ...wave3NearbySeeds, ...wave4NearbySeeds, ...wave5NearbySeeds, ...wave6NearbySeeds, ...wave7NearbySeeds, ...wave8NearbySeeds];")
...
det_block = "  // ===== Wave 8 verified detail pages (2026-06-15) =====\n" + "\n".join(f"  {j(s)}," for s in detail_slugs)
idx = det_src.rfind("\n] as const;")
...
wt_lines = ["  // ===== Wave 8 (2026-06-15) ====="]
vg_lines = ["  // ===== Wave 8 (2026-06-15) ====="]
