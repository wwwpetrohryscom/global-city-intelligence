...
import { germanAlternate } from "@/lib/i18n/german-pairs";
import {
  absoluteUrl,
  siteUrl,
  arrivalRoute,
...
export function createMetadata({
  ...
  /*
   * THE GERMAN ALTERNATE, WHERE ONE EXISTS.
   *
   * Emitted from a committed contract generated by the German edition's own
   * route manifest — never inferred from a country or a city name. 240 of
   * Germany's 257 cities have a German profile and 18 of its 1,542 module
   * pages do, so anything derived would advertise thousands of alternates
   * pointing at pages that are not published.
   *
   * THE GERMAN URL KEEPS ITS TRAILING SLASH. That application runs with
   * `trailingSlash: true` and this one with false, so the asymmetry is what
   * makes the target return 200 rather than a redirect. DE-1 hit the mirror
   * image of this — Next appended a slash to Main's unslashed URL and pointed
   * an alternate at a 301 — so the behaviour was measured here rather than
   * assumed: Next 15.5.15 leaves a fully-qualified absolute URL alone.
   *
   * x-default is the English page, matching what the German side already
   * emits, so a cluster reads the same from either end.
   */
  const german = germanAlternate(path);
